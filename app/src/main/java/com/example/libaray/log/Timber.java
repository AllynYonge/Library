 ...
 * 该类要在Application的onCreate回调方法中调用Timber.plant(Tree)内置有一个DebugTree，我们可以直接用
 * 我们也可以自定义一个Tree，调用流程是Timber.v() ——> TREE_OF_SOULS.v(系统内置的Tree) ——> 自定义的Tree.v
 * ——> prepareLog() prepareLog ——> 自定义的Tree.log*/
public final class Timber {
    ...
    public static abstract class Tree {
        ...
        /** 判断是否要打印日志，若为true则打印日志 */
        protected boolean isLoggable(int priority) {
            return true;
        }

        /**
         * 在打印日志之前都先调用这个方法，所以你在这里可以进行加工处理
         * @param priority
         * @param t
         * @param message
         * @param args
    ...
    public static class DebugTree extends Tree {
        ...
        @Override protected void log(int priority, String tag, String message, Throwable t) {
            if (message.length() < MAX_LOG_LENGTH) {
                //Log.ASSERT是android4.0以上的增加的
                if (priority == Log.ASSERT) {
                    //该方法输出一个日志.在输出日志的同时，它会把此处代码此时的执行路径(调用栈)打印出来。
                    Log.wtf(tag, message);
                } else {
