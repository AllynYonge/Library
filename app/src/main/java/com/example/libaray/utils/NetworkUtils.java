...
public class NetworkUtils {
    ...
    public static boolean printNetworkInfo(Context context) {
        ...
        if (connectivity != null) {
            ...
            Timber.i(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
            Timber.i(TAG, "getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
                    Timber.i(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
                    Timber.i(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
                    Timber.i(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
                    Timber.i(TAG, "NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
                Timber.i(TAG, "\n");
            } else {
                Timber.i(TAG, "getAllNetworkInfo is null");
            }
        }
        return false;
    }
    ...
    public static int getConnectedTypeINT(Context context) {
        NetworkInfo net = getConnectivityManager(context).getActiveNetworkInfo();
        if (net != null) {
            Timber.i(TAG, "NetworkInfo: " + net.toString());
            return net.getType();
        }
        return -1;
    }
